...
export function ClearReadingItemButton({ paperId, title }: { paperId: string; title: string }) {
  ...
  return (
    <button
      ...
      className="absolute right-2 top-2 rounded-full p-1 text-muted-foreground transition-opacity hover:bg-tint hover:text-ink focus-visible:opacity-100 disabled:opacity-50 sm:opacity-0 sm:group-hover:opacity-100"
    >
...
export function ClearReadingAllButton() {
  ...
  return (
    <button
      ...
      className="rounded-full px-2.5 py-1 font-mono text-[11px] text-faint transition-colors hover:bg-tint hover:text-ink disabled:opacity-50"
    >
